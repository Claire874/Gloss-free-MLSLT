...
class Batch:
    ...
    def __init__(
        ...
        if hasattr(torch_batch, "txt"):
            ...
            #print(txt,txt_lengths,len(txt))
            txt_ori = txt
            self.txt_ori = txt_ori
            ...
            self.txt = txt[:, 1:]
            #print('~~~!!!!~~~',self.txt_ori,self.txt_ori.shape,self.txt.shape,self.txt)
    ...
    def _make_cuda(self):
        ...
        if self.txt_input is not None:
            self.txt_ori = self.txt_ori.cuda()
    ...
    def sort_by_sgn_lengths(self):
        ...
        if self.txt is not None:
            self.txt_ori = self.txt_ori[perm_index]
