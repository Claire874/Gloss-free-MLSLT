...
#from signjoey.search_jointdecoding_combine_new import joint_beam_search
from signjoey.search_jointdecoding_new import joint_beam_search
#from signjoey.search_jointdecoding import joint_beam_search, ctc_score
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        self.text_output_layer = text_output_layer
        self.do_recognition = do_recognition
        self.do_translation = do_translation
        self.do_joint = do_joint
    ...
    def forward(
        ...
        """
        encoder_output, encoder_hidden, specific_layer_output = self.encode(
        ...
        if self.do_recognition:
            ...
            # N x T x C
            #gloss_scores = self.gloss_output_layer(encoder_output) #may change here
            gloss_scores = self.gloss_output_layer(specific_layer_output)
        ...
        if self.do_joint:
            text_scores = self.text_output_layer(encoder_output)
            text_probabilities = text_scores.log_softmax(2)
            text_probabilities = text_probabilities.permute(1, 0, 2)
        else:
        ...
        return decoder_outputs, gloss_probabilities,text_probabilities
    ...
    def get_loss_for_batch(
        ...
        :param joint_loss_function: Loss Function (CTC) for translation
        :param joint_loss_weight: Weight for joint loss
        ...
        # Do a forward pass
        decoder_outputs, gloss_probabilities, text_probabilities = self.forward(
            sgn=batch.sgn,
        ...
        if self.do_joint:
            assert text_probabilities is not None
            #print('~~~~~~~~~text_probabilities~~~~~~~',text_probabilities.shape)
            #print('~~~~~~~~~ batch.txt~~~~~~~',batch.txt.shape)
            #print('~~~~~~~~~ batch.txt_ori~~~~~~~', batch.txt_ori.shape)
            #print('~~~~batch.gls~~~',batch.gls.shape)
            #print('~~~~~~~~batch.sgn_lengths.long()~',batch.sgn_lengths.long())
            #print('~~~~~~~~batch.txt_lengths.long()~',batch.txt_lengths.long())
            joint_loss = (
                joint_loss_function(
                    text_probabilities,
                    #batch.gls,
        ...
        return recognition_loss, joint_loss, translation_loss
    ...
    def run_batch(
        ...
        :param translation_beam_size: size of the beam for translation beam search
            if 1 use greedy
        :param joint_beam_size: size of the beam for translation beam search
            if 1 use greedy
...
#先不改decode的部分
        ...
        if self.do_translation:
            ...
            #joint ctc/attention decoding
                    ...
                    #print('doing beam-searching')
                    stacked_txt_output, stacked_attention_scores = joint_beam_search(
                        ...
                        bos_index=self.txt_bos_index,
                        decoder=self.decoder,
                        ctc_layer=self.text_output_layer,
                    )
...
def build_model(
    ...
    :param do_recognition: flag to build the model with recognition output.
    :param do_joint: flag to build the model with translation
    :param do_translation: flag to build the model with translation decoder.
    """
    ...
    if do_joint:
        text_output_layer = nn.Linear(encoder.output_size,len(txt_vocab))
    else:
        text_output_layer = None
