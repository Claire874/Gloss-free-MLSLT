...
#from signjoey.metrics_sacrebleu.py import bleu, chrf, rouge, wer_list
...
def validate_on_data(
    ...
    do_joint: bool,
    joint_loss_function: torch.nn.Module,
    joint_loss_weight: int,
    do_translation: bool,
    ...
    :param joint_loss_function: joint loss function (CTC)
    :param recognition_loss_weight: CTC loss weight
    :param joint_loss_weight: CTC loss weight
    ...
    :param do_joint：flag for joint loss
    ...
    with torch.no_grad():
        ...
        for valid_batch in iter(valid_iter):
            ...
            batch_recognition_loss, batch_joint_loss, batch_translation_loss = model.get_loss_for_batch(
                batch=batch,
            ...
            if do_joint:
                total_joint_loss += batch_joint_loss
            if do_translation:
        ...
        if do_joint:
            if(
                joint_loss_function is not None
                and joint_loss_weight !=0
            ):
                valid_joint_loss = total_joint_loss
        ...
        if do_translation:
            ...
            # TXT Metrics
            txt_bleu = bleu(hypotheses=txt_hyp,references=txt_ref)
    ...
    if do_joint:
        results["valid_joint_loss"] = valid_joint_loss
    if do_translation:
...
def test(
    ...
    if do_joint:
        joint_loss_function = torch.nn.CTCLoss(
            blank=0,zero_infinity=True
        )
        if use_cuda:
            joint_loss_function.cuda()
    if do_translation:
    ...
    if do_recognition:
        ...
        for rbw in recognition_beam_sizes:
            ...
            dev_recognition_results[rbw] = validate_on_data(
                ...
                do_joint=do_joint,
                joint_loss_function=joint_loss_function,
                joint_loss_weight=1 if do_joint else None,
        ...
        for tbw in translation_beam_sizes:
            ...
            for ta in translation_beam_alphas:
                dev_translation_results[tbw][ta] = validate_on_data(
                    ...
                    # Joint parameters
                    do_joint=do_joint,
                    joint_loss_function=joint_loss_function if do_joint else None,
                    joint_loss_weight=1 if do_joint else None,
                    do_translation=do_translation,
                    translation_loss_function=translation_loss_function,
