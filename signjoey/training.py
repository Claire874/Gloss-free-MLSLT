...
from signjoey.loss_labelsmoothing import SmoothCTCLoss
...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.do_joint = (
                config["training"].get("joint_loss_weight", 1.0) > 0.0
        )
        self.do_translation = (
        ...
        if self.do_joint:
            self._get_joint_params(train_config=train_config)
    ...
    def _get_joint_params(self, train_config) -> None:
        self.gls_silence_token = self.model.gls_vocab.stoi[SIL_TOKEN]
        assert self.gls_silence_token == 0
        self.joint_loss_function = torch.nn.CTCLoss(
           blank=self.gls_silence_token, zero_infinity=True
         )
        #torch.nn.CTCLoss(
            #reduction='mean', blank=0, zero_infinity=True)
        self.joint_loss_weight = train_config.get("joint_loss_weight", 1.0)
    ...
    def init_from_checkpoint(
        ...
        pretrained_model_dict = model_checkpoint["model_state"]
        for k in list(pretrained_model_dict):
            del_list = ['decoder.output_layer', "gloss_output_layer","text","txt_emb"]#'text','txt_emb',,'decoder','encoder.layers.2','encoder.layers.1','gloss','gls']
            for i in del_list:
                 if i in k:
                     print('delete')
                     pretrained_model_dict.pop(k)
            #if 'encoder' in k or 'gloss' in k or 'sgn_emb' or 'decoder'in k:
            #if 'decoder' in k or 'text' in k or 'txt_emb' in k or 'encoder.layers.0' in k or 'encoder.layers.1' in k or 'gloss':
            #    print('delete')
            #    pretrained_model_dict.pop(k)
                
        
        print("~~~~~~~~~~~keys~~~~~~~~~~~~~~",pretrained_model_dict.keys())
        #del pretrained_model_dict["txt_embed.lut.weight"]
        #del pretrained_model_dict["decoder.output_layer.weight"]
        #del pretrained_model_dict["txt_embed.lut.weight"]
        #del pretrained_model_dict["text_output_layer.weight"]
        #del pretrained_model_dict["text_output_layer.bias"]
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) -> None:
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                recognition_loss, joint_loss, translation_loss = self._train_batch(
                    batch, update=update
                )
                ...
                if self.do_joint:
                    self.tb_writer.add_scalar("train/train_joint_loss", joint_loss, self.steps
                    )
                    epoch_joint_loss += joint_loss.detach().cpu().numpy()
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    val_res = validate_on_data(
                        ...
                        # Joint Parameters
                        do_joint = self.do_joint,
                        joint_loss_function = self.joint_loss_function
                        if self.do_joint
                        else None,
                    ...
                    if self.do_joint:
                        # Log Losses and ppl
                        self.tb_writer.add_scalar(
                            "valid/valid_joint_loss",
                            val_res["valid_joint_loss"],
                            self.steps,
                        )
                    ...
                    self._add_report(
                        ...
                        valid_joint_loss = val_res["valid_joint_loss"]
                        if self.do_joint
                        else None,
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> (Tensor, Tensor):
        ...
        recognition_loss, joint_loss, translation_loss = self.model.get_loss_for_batch(
            batch=batch,
            recognition_loss_function=self.recognition_loss_function
            if self.do_recognition
            else None,
        ...
        if self.do_joint:
            normalized_joint_loss = joint_loss / self.batch_multiplier
        else:
            normalized_joint_loss = 0

        total_loss = normalized_recognition_loss + normalized_joint_loss + normalized_translation_loss
        # compute gradients
        total_loss.backward()
        ...
        return normalized_recognition_loss, normalized_joint_loss, normalized_translation_loss
        ...
        with open(self.valid_report_file, "a", encoding="utf-8") as opened_file:
            opened_file.write(
                ...
                "LR: {:.8f}\t{}\n".format(
                    ...
                    valid_joint_loss if self.do_joint else -1,
...
def train(cfg_file: str) -> None:
    ...
    do_joint = cfg["training"].get("joint_loss_weight", 1.0) > 0.0
