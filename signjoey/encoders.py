...
class TransformerEncoder(Encoder):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, mask)
            if i == len(self.layers)-math.ceil(len(self.layers)/2)+1:
                specific_layer = x
                #print('layer',i)
                #print('~~~~~~self.layer_norm(specific_layer).shape~~~~~~~~',i, self.layer_norm(specific_layer))

        #print('~~~~~~self.layer_norm(x).shape~~~~~~~~',len(self.layers),self.layer_norm(x),self.layer_norm(x).equal(self.layer_norm(specific_layer)) )

        return self.layer_norm(x), None, self.layer_norm(specific_layer)
